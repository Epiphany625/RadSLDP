#!/usr/bin/env bash
# Generate predictions on the ReXGradient test split for all 12 trained
# checkpoints (3 models x 4 regimes) produced by ../training/run_training_rex.sh.
set -euo pipefail
...
for MODEL in "${MODELS[@]}"; do
  ...
  for TAG in "${REGIMES[@]}"; do
    CKPT="${CKPT_ROOT}/${NAME}-rex-${TAG}_final"
    PRED_FILE="${PRED_BASE}/${NAME}-rex-${TAG}"
    echo "===================================================="
    echo "PREDICT REX | ${TAG} | ${MODEL}"
